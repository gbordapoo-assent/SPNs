...
1. **Builds a week-by-week calendar** using a recursive CTE, starting 12 months ago and ending today.
2. **Left joins** each week to `tblPart` to find parts that:
   - Were created **before the end of that week**, and
   - Are **still active** (`DateDeleted IS NULL`).
...
- The script expects the `AssentDataManitouAmericas` database and a `tblPart` table with:
  - `partID`
  - `SupplierPartNumber`
  - `DateCreated`
  - `DateDeleted`
